...
| Reader TTS | TTS-хоткей `cmd_l+alt+t` или `📖 Reader -> 🔊 Запустить TTS` | Текстовый тип из clipboard, backend Apple/MLX с Apple AVSpeech по умолчанию, Apple voice id или встроенный MLX-голос `Русский спокойный диктор`, интонация TTS, скорость, лимит длительности, флаг LLM-предобработки | Read-only чтение clipboard, LLM-предобработка или fallback, чистка markdown/URL/кода, ограничение длины | Локальное озвучивание через Apple AVSpeech или MLX Qwen3-TTS VoiceDesign; clipboard не меняется |
| Zipper | Zipper-хоткей `ctrl+shift+alt+z` или `🧷 Zipper -> Запустить Zipper` | Аудио команды, конфиг Zipper, текущая MLX LLM, память, инструменты, clipboard/URL/CLI/MCP при явном вызове | ASR без автовставки, инфраструктурный агентский runtime с Hermes function calling для Qwen и ReAct для остальных LM, prompt-контракт готовой голосовой фразы, allowlist-инструменты, сохранение событий и суммаризация памяти | Голосовой ответ через текущий TTS-профиль, текстовое окно, debug-события, изменения только через выбранные инструменты |
...
3. `ModelManager` и единый runtime-сервис на старте прогревают только настроенную
   MLX TTS-модель reader, если её snapshot уже есть в локальном cache.
4. Выбранные ASR и LLM/VLM остаются ленивыми: ASR загружается при первой
   обычной диктовке, записи для `Whisper -> LLM` или Zipper-команде, а
   LLM/VLM — при первом сценарии `Whisper -> LLM`, reader-предобработке или
   Zipper-команде.
5. При смене ASR, LLM/VLM или MLX TTS старый `model_id` освобождается, а новая
   выбранная модель прогревается в фоне.
6. `DictationApp` получает зависимости через конструктор и создаёт use case-слой.
7. `StatusBarApp` подписывается на snapshot приложения и показывает текущее
   состояние в menu bar.
8. `HotkeyDispatcher` принимает глобальные сочетания клавиш через `CGEventTap`.
9. Каждый пользовательский сценарий меняет state приложения и публикует новый
   snapshot для UI.
10. Ошибки сценариев должны заканчиваться понятным уведомлением и fallback-ом,
...
MLX TTS по умолчанию использует `mlx-community/Qwen3-TTS-12Hz-1.7B-VoiceDesign-4bit`,
китайскую инструкцию описания русского дикторского голоса и фиксированный
`lang_code="russian"`.
