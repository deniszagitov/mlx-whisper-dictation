...
| Reader TTS | TTS-хоткей `cmd_l+alt+t` или `📖 Reader -> 🔊 Запустить TTS` | Текстовый тип из clipboard, backend Apple/MLX, Apple voice id или встроенный MLX-голос `Русский спокойный диктор`, интонация TTS, скорость, лимит длительности, флаг LLM-предобработки | Read-only чтение clipboard, LLM-предобработка или fallback, чистка markdown/URL/кода, ограничение длины | Локальное озвучивание через Apple AVSpeech или MLX Qwen3-TTS VoiceDesign; clipboard не меняется |
| Zipper | Zipper-хоткей `ctrl+shift+alt+z` или `🧷 Zipper -> Запустить Zipper` | Аудио команды, конфиг Zipper, текущая MLX LLM, память, инструменты, clipboard/URL/CLI/MCP при явном вызове | ASR без автовставки, инфраструктурный агентский runtime с Hermes function calling для Qwen и ReAct для остальных LM, allowlist-инструменты, сохранение событий и суммаризация памяти, TTS-подготовка голосовых ответов без цифр и технической разметки | Голосовой ответ через текущий TTS-профиль, текстовое окно, debug-события, изменения только через выбранные инструменты |
