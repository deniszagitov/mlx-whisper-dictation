...
| Zipper | Zipper-хоткей `ctrl+shift+alt+z` или `🧷 Zipper -> Запустить Zipper` | Аудио команды, конфиг Zipper, текущая MLX LLM, память, инструменты, clipboard/URL/CLI/MCP при явном вызове | ASR без автовставки, инфраструктурный агентский runtime с Hermes function calling для Qwen и ReAct для остальных LM, allowlist-инструменты, сохранение событий и суммаризация памяти | Голосовой ответ, текстовое окно, debug-события, изменения только через выбранные инструменты |
