...
| Zipper | Zipper-хоткей `ctrl+shift+alt+z` или `🧷 Zipper -> Запустить Zipper` | Аудио команды, конфиг Zipper, текущая MLX LLM, память, инструменты, clipboard/URL/CLI/MCP при явном вызове | ASR без автовставки, инфраструктурный агентский runtime с Hermes function calling для Qwen и ReAct для остальных LM, allowlist-инструменты, сохранение событий и суммаризация памяти | Голосовой ответ через текущий TTS-профиль, текстовое окно, debug-события, изменения только через выбранные инструменты |
