...
| Reader TTS | TTS-хоткей `cmd_l+alt+t` или `📖 Reader -> 🔊 Запустить TTS` | Текстовый тип из clipboard, backend Apple/MLX, Apple voice id или встроенный MLX-голос, интонация TTS, скорость, лимит длительности, флаг LLM-предобработки | Read-only чтение clipboard, LLM-предобработка или fallback, чистка markdown/URL/кода, ограничение длины | Локальное озвучивание через Apple AVSpeech или MLX Qwen3-TTS; clipboard не меняется |
