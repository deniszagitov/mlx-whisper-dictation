...
5. Перед вставкой к распознанному тексту применяется настраиваемая постобработка: первая буква может переводиться в верхний регистр, а конечная точка у единственного предложения может автоматически убираться.
...
В отдельном подменю `✨ Постобработка текста` доступны два независимых правила, оба включены по умолчанию:
