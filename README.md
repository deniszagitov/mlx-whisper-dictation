...
- **LLM-пайплайн** использует буфер обмена отдельно от обычной диктовки: при включённом пункте «Буфер обмена для LLM» текст из буфера может стать входом для LLM, а итог обработки записывается обратно в буфер.
...
## LLM hotkey и буфер обмена

LLM-пайплайн работает отдельно от обычной диктовки.

- Обычный hotkey Whisper всегда берёт текст только из микрофона.
- Переключатель «Буфер обмена (Cmd+V)» относится только к обычной диктовке и задаёт способ вставки текста в активное приложение.
- Переключатель «Буфер обмена для LLM» относится только к LLM-пайплайну.

Если «Буфер обмена для LLM» включён, логика такая:

1. При нажатии LLM-hotkey приложение читает текст из буфера обмена.
2. Если в буфере есть текст, именно он становится основным входом для LLM.
3. Если пользователь дополнительно что-то диктует, эта фраза становится дополнительной инструкцией.
4. Если пользователь молчит, но буфер не пуст, LLM всё равно вызывается и применяет выбранный системный промпт к тексту из буфера.
5. Готовый результат записывается обратно в буфер обмена.

Если «Буфер обмена для LLM» выключён, LLM работает как голосовой пайплайн без участия буфера обмена.

Примеры:

- пресет «Переведи на English» переведёт текст из буфера на английский;
- пресет «Переведи на русский» переведёт текст из буфера на русский;
- пресет «Исправь текст» исправит пунктуацию, регистр и пробелы в тексте из буфера.
