...
Во время сборки MkDocs автоматически запускает pre-build hook `scripts/generate_docs.py`, который читает `whisper-dictation.py` и `setup.py`, вытаскивает module/class/function docstring и обновляет API-страницы в каталоге `docs/`.
